...
/// @notice Thrown when the sender address has already generated its maximum proceeds
error MaximumSenderGeneratedProceedsReached();
...
contract Doppler is BaseHook {
    ...
    /// @notice Maximum proceeds an address can generate
    uint256 public maximumSenderGeneratedProceeds;

    /// @notice Proceeds generated by each sender address
    mapping(address sender => uint256 proceeds) public senderGeneratedProceeds;

    /// @notice Router contracts which can be trusted to return a valid msgSender()
    ...
    function unlockCallback(
        ...
        if (isMigration) {
            return HelperLib.unlockCallbackMigration(key, sender, poolManager, numPDSlugs, positions, isToken0);
        }
    ...
    /**
     * @notice Sets the verified status of a router contract
     * @dev Verified routers are trusted to return a valid msgSender()
     * @param router Address of the router contract
     * @param isVerified True if the router is verified
     */
    function setVerifiedRouter(address router, bool isVerified) external {
        if (msg.sender != initializer) revert SenderNotInitializer();
        verifiedRouters[router] = isVerified;
    }

    function setMaximumSenderGeneratedProceeds(
        uint256 maximumSenderGeneratedProceeds_
    ) external {
        if (msg.sender != initializer) revert SenderNotInitializer();
        maximumSenderGeneratedProceeds = maximumSenderGeneratedProceeds_;
    }

    function _updateSenderGeneratedProceeds(address sender, uint256 amount) internal {
        // Skip check for self
        if (sender == address(this)) return;

        address initialSender = verifiedRouters[sender] ? IMsgSender(sender).msgSender() : sender;

        // Skip check for self
        if (initialSender == address(this)) return;
...
library HelperLib {
    ...
    function unlockCallbackMigration(
        PoolKey memory key,
        address sender,
        IPoolManager poolManager,
        uint256 numPDSlugs,
        mapping(bytes32 salt => Position position) storage positions,
        bool isToken0
